    ...
    \section{度量空间} % uuid:"Metric-Space"
        ...
        \subsection{基本概念} % uuid:"Metric-Space-Basic"
            ...
            \begin{lma}
                ...
                []
                [猫猫]
                设 \((S,d)\) 是\度量空间, \(K\subseteq S\), \(\紧集[K]\), \(\mathcal{O}\) 是 \(K\) 的开覆盖, 则: 
                \[\exists\delta:\R^+,\forall X\subseteq K, \diam(X)<\delta\implies\exists O\in\mathcal{O}, X\subseteq O\]
            \end{lma}
            
            \begin{rmk}
                [猫猫]
                Lebesgue 数是紧集开覆盖的性质, 它描述了开覆盖的``粒度''. 
            \end{rmk}
    ...
    \section{一致收敛}
        ...
        \subsection{几种实数空间上的一致收敛}
            ...
            \begin{dfn}
                ...
                []
                [Dedicatia]
                设函数项级数 \(\sum_{n=1}^{\infty} u_n(x), x\in E\subseteq\R\) , 令 \(S_n(x) = \sum_{k=1}^{n} u_k(x)\). 定义该函数项级数在 \(E\) 上\textbf{一致收敛}, 如果函数列 \(\{S_n(x)\}\) 在 \(E\) 上\hyperref[dfn:Real.UniConv]{一致收敛}于某个函数 \(S(x)\).
            \end{dfn}
            ...
            \begin{dfn}
                ...
                [Dedicatia]
                \textbf{无穷限反常积分}:\\
                对于定义在 \([a,+\infty)\times Y\) 上的二元连续函数 $f$, 考虑积分 \(I(y) = \int_a^{\infty} f(x, y) \dd x\). 令 \(F_A(y) = \int_a^{A} f(x, y) \dd x\), 定义\textbf{一致收敛}, 当且仅当对于任意 \(\varepsilon > 0\)，存在与 \(y\) 无关的 \(A_0(\varepsilon) > a\)，使得当 \(A > A_0(\varepsilon)\) 时，对一切 \(y \in Y\)，有：
                \[\forall\varepsilon>0, \exists A_0, \st A>A_0, \left| \int_A^{\infty} f(x, y) \dd x \right| < \varepsilon\]
                \textbf{瑕积分}:\\
                对于定义在 \([a,b)\times Y\) 上的二元连续函数 $f$, 其中 \(f(x, y)\) 在 \(x=b\) 附近无界. 令 \(F_{\eta}(y) = \int_a^{b-\eta} f(x, y) \, dx\), 称该瑕积分在 \(Y\) 上关于 \(y\) 一致收敛, 如果 \(F_{\eta}(y)\) 在 \(\eta \to 0^+\) 时关于 \(y \in Y\) 一致收敛于 \(I(y)\).
            \end{dfn}
        ...
        \subsection{一致收敛的性质}
            ...
            \begin{ppt}
                [Real.UniConvOfDiff]
                {函数列逐点收敛且导数一致收敛则保持可微性}
                []
                [Dedicatia]
                若函数列 $\{f_n\}$ 满足: 每一个函数 $f_n(x)$ 在 $[a,b]$ 上可导且 $f'_n(x)$ 一致收敛于某个函数 $g$, 且 $\exists\,x_0\in[a,b]\st f_n(x_0)$ 收敛, 那么 $\{f_n\}$ 一致收敛于某个连续可微的函数 $f$ 并且 $f'=g$.
            \end{ppt}
            ...
            \begin{cxmp}
                ...
                则 $f_n$ 在 $(-\infty,+\infty)$ 上无穷次可微, 且 $f\rightrightarrows 0$. 但
                \[f'_n(x)=\sqrt{n}\cos nx\]
                显然在 $(-\infty,+\infty)$ 上处处不收敛.
            \end{cxmp}
            \begin{cxmp}
                [213301]
                {一致收敛的的可微函数列导函数不一定可微}
                []
                [Dedicatia]
                著名的Weierstrass函数就是由函数项级数构造而来的. 函数项级数可以视作函数列:
                \[W_n(x)=\sum_{k=0}^nb^k\cos(a^k\pi x)\]
                这里 $0<b<1$ 用于控制收敛, $a$ 是正奇数, $ab>1+\frac{3\pi}{2}$. 显然 $W_n(x)$ 是无穷次可微函数, 且 $W_n$ 可以一致收敛于 $W$, $W$ 是连续函数, 但不是可微函数.
            \end{cxmp}
            \begin{cxmp}
                [213801]
                {一致收敛的可微函数列的求导与极限不能交换}
                []
                [Dedicatia]
                设函数列
                \[f_n(x)=\frac{1}{n}\arctan x^n\qquad x\in(-\infty,+\infty)\]
                显然对所有的 $x$:
                \[|f_n(x)-0|=\vqty{\frac{1}{n}\arctan x^n}\leq\frac{\pi}{2n}\to 0\]
                由此可见函数列 $f_n\rightrightarrows 0$;\\
                另一方面, 
                \[\qty(\lim_{n\to\infty}f_n(x))\bigg|_{x=1}=0\]
                \[\lim_{n\to\infty}f'_n(1)=\lim_{n\to\infty}\qty(\frac{x^{n-1}}{1+x^{2n}})\bigg|_{x=1}=\frac{1}{2}.\]
                这说明即使原函数列一致收敛, 其极限的导函数也不一定等于其导函数的极限.
            \end{cxmp}
            ...
            \begin{ppt}
                ...
                []
                [Dedicatia]
                设 $I\subseteq\R$, $E\subseteq\R$, $\varphi(x,y):I\times E\to \R$, $y_0\in E'$. 若对于每个 $y\in E$, $\varphi(x,y)$ 关于 $x$ 在 $I$ 上连续, 在 $y\to y_0$ 时在 $I$ 上关于 $x$ 一致收敛到 $f(x)$, 那么 $f(x)$ 在 $I$ 上连续. 且 $\forall\,x_0\in I$
                \[\lim_{x\to x_0}\lim_{y\to y_0}\varphi(x,y)=\lim_{y\to y_0}\lim_{x\to x_0}\varphi(x,y).\]
            \end{ppt}
        ...
        \subsection{等度连续与序列紧致性}
            ...
            \begin{thm}
                ...
                []
                [Dedicatia]
                设 $X$ 是一个度量空间的闭凸子集, $f:X\to X$ 是连续映射, $f(X)$ 是序列紧的, 那么 $f$ 在 $X$ 上至少有一个\hyperref[dfn:Fixed]{不动点}.
            \end{thm}
            ...
            \begin{thm}
                ...
                []
                [Dedicatia]
                设 $\{f_n\}$ 是定义在可数集 $E:=\{x_1,x_2,\cdots,x_n,\cdots\}$ 上的\hyperref[dfn:PointwiseBounded]{逐点有界}的函数列, 那么它存在\hyperref[dfn:Metric.PointwiseConv]{逐点收敛}的子列. 
            \end{thm}
            ...
            \begin{cxmp}
                ...
                [Dedicatia]
                考虑函数列 
                \[f_n(x)=\sin nx,\quad (x\in[0,2\pi], n\in\N)\]
                显然这是\hyperref[dfn:UniBounded]{一致有界的}. 但是它不存在\hyperref[dfn:Metric.PointwiseConv]{逐点收敛}的子列.
            \end{cxmp}

            \begin{cxmp}
                [205701]
                {函数列一致有界且逐点收敛不一定有一致收敛的子列}
                []
                [Dedicatia]
                设 
                \[f_n(x)=\frac{x^2}{x^2+(1-nx)^2},\quad x\in[0,1],\,n\in\N^*\]
                则对任何 $x\in[0,1]$ 和 $n$, $|f_n(x)|\leq 1$ 恒成立. 即 $f_n$ 一致有界;\\
                又因为
                \[\forall x_0\in[0,1],\lim_{n\to\infty}f_n(x_0)=0 \]
                即 $f$ 逐点收敛到0; 但是
                \[f_n\qty(\frac{1}{n})=1\]
                可见 $f_n$ 的任何子列都无法一致收敛到0.
            \end{cxmp}
            ...
            \begin{ppt}
                {等度连续且逐点收敛则一致收敛}
                Let $\{f_n\}$ be a function series defined over $E\subseteq X$. If $\{f_n\}$ is \hyperref[dfn:EquiCont]{equicontinuous} and \hyperref[dfn:Real.PointwiseConv]{pointwise convergent} to $f$ over $E$, then $f_n$ is \hyperref[dfn:Metric.UniConv]{uniformally convergent} to $f$.
            \end{ppt}
            \begin{thm}
                [Arzela-Ascoli]
                {Arzela-Ascoli定理}
            ...
            \begin{rmk}
                [Dedicatia]
                \hyperref[thm:Arzela-Ascoli]{Arzela-Ascoli定理}沟通了等度连续与函数空间上的序列紧性之间的关系。从拓扑空间的角度看，如果一个函数序列在紧集 $X$ 上是一致有界且等度连续的，那么该函数列作为拓扑空间必定是序列紧的。
            \end{rmk}
        ...
        \subsection{多项式的一致逼近}
            ...
            \begin{thm}
                [WeierstrassApprox]
                {Weierstrass逼近定理}
                [Weierstrass Apporximation Theorem]
                [Dedicatia]
                Let a \hyperref[dfn:Cont]{continuous} function $f:[a,b]\to\R$. Then there is a polynomial sequence $\{P_n\}$ such that
                \[P_n\rightrightarrows f\]
                over $[a,b]$.
            \end{thm}
        ...
        \subsection{卷积及其应用}
            ...
            \begin{dfn}
                ...
                [Convolution]
                [Dedicatia]
                设函数 $f,g:\R\to\R$ 是局部Riemann可积的. 定义其\textbf{卷积}为含参变量积分
                \[(f\ast g)(x):=\int_{\R}f(t)g(x-t)\dd{t}.\]
                定义 $g(x-t)$ 为 $f$ 的\textbf{卷积核}.
            \end{dfn}
            ...
            \begin{dfn}
                ...
                [Support Set]
                [Dedicatia]
                设函数 $f:\R\to\R$. 定义其\textbf{支撑集/支集}为
                \[\supp f := \overline{\{x:x\in\R,f(x)\neq 0\}}.\]
                将 $f$ 具有区间 $[a,b]$ 上的紧支撑集记作 $f\in C_0([a,b])$.
            \end{dfn}
            ...
            \begin{dfn}
                ...
                [Smooth]
                [Dedicatia]
                定义 $f:E\subseteq\R\to\R$ 是 $n$ 阶光滑函数当且仅当 $f$ 具有 $n$ 阶连续的导函数. 记作 $f\in C(E)$.\\
                定义 $f$ 是光滑函数（省略 $n$）指的是当且仅当 $f\in C^{\infty}(E)$.
            \end{dfn}
            ...
            \begin{ppt}
                ...
                []
                [Dedicatia]
                定义函数 $f,g$ 的卷积为 $f\ast g$. 若该卷积收敛, 则具有以下性质:
                \begin{enumerate}
                    \item 对称性: $f\ast g=g\ast f$;
                    \item 支撑集可加性: 设 $\supp f\subseteq \Icc{a}{b}, \supp g\subseteq \Icc{c}{d}$, 则 $\supp(f\ast g)\subseteq \Icc{a+c}{b+d}$;
                    \item 平移不变性: 定义 $T_{x_0}(f(x))=f(x-x_0)$. 那么 $T_{x_0}(f\ast g)=(T_{x_0}(f))\ast g=f\ast(T_{x_0}(g))$.
                \end{enumerate}
            \end{ppt}
            ...
            \begin{crl}
                ...
                []
                [Dedicatia]
                设函数 $f,g:\R\to\R$ 是局部Riemann可积的. 若 $|f|^2, |g|^2$ 均在 $\R$ 上Riemann可积, 则卷积收敛.
            \end{crl}
            \begin{crl}
                [105501]
                {卷积收敛的判定定理2}
                []
                [Dedicatia]
                设函数 $f,g:\R\to\R$ 是局部Riemann可积的. 若 $\supp f$ 或 $\supp g$ 是紧集, 则卷积收敛.
            \end{crl}
            \begin{crl}
                [110101]
                {卷积收敛的判定定理3}
                []
                [Dedicatia]
                设函数 $f,g:\R\to\R$ 是局部Riemann可积的. 若 $f$ 和 $g$ 一个绝对Riemann可积, 另一个有界, 则卷积收敛.
            \end{crl}
            ...
            \begin{thm}
                ...
                []
                [Dedicatia]
                设函数 $f,g:\R\to\R$ 是局部Riemann可积的. $g\in C_0^n(\R)$. 那么 $f\ast g\in C^n(\R)$, $\forall 0<k\leq n, (f\ast g)^{(k)}=(f\ast g^{(k)})(x)$.
                \[\dv[k]{x}\int_{\R}f(t)g(x-t)\dd{t}=\int_{\R}f(t)\dv[k]{g(x-t)}{x}\dd{t}.\]
            \end{thm}
            ...
            \begin{dfn}
                ...
                [Dedicatia]
                定义函数 $g:[a,b]\subseteq\R\to\R$ 是一个\textbf{磨光核}, 当且仅当 $g$ 满足以下条件:
                \begin{enumerate}
                    \item 非负性: $g\geq 0$;
                    \item 光滑性: $g\in C^{\infty}([a,b])$;
                    \item 规范性: $\int_{\R}g(x)\dd{x}=1$;
                    \item 紧支撑: $g\in C_0([a,b])$.
                \end{enumerate}
            \end{dfn}

            \begin{rmk}
                [Dedicatia]
                局部可积的函数可以与一个磨光核作卷积, 得到的新函数保持光滑性. 这就是磨光核的名称由来.
            \end{rmk}
            
            \begin{dfn}
                [Delta-Approx-Sequence]
                {$\delta$-函数逼近列}
                []
                [Dedicatia]
                定义函数列 $\{f_n\}$ 为一个 $\delta$-函数逼近列, 若满足
                \begin{enumerate}
                    \item 非负性: $\forall n\in\N^*, f_n\geq 0$;
                    \item 规范性: $\forall n\in\N^*, \int_{\R}f_n(x)\dd{x}=1$;
                    \item 集中在0附近: 对于 $x=0$ 的任意邻域 $U(0)$: \[\lim_{n\to\infty}\int_{U(0)}f_n(x)\dd{x}=1.\]
                \end{enumerate}
            \end{dfn}
            ...
            \begin{dfn}
                ...
                []
                [Dedicatia]
                设 $\epsilon\in\Lambda$ 是参数, 定义函数列 $\{f_{\epsilon}\}$ 为一个 $\delta$-函数逼近列, 若满足
                \begin{enumerate}
                    \item 非负性: $\forall \epsilon\in\Lambda, f_\epsilon\geq 0$;
                    \item 规范性: $\forall \epsilon\in\Lambda, \int_{\R}f_\epsilon(x)\dd{x}=1$;
                    \item 集中在0附近: 对于 $x=0$ 的任意邻域 $U(0)$: \[\lim_{\epsilon\to\epsilon_0}\int_{U(0)}f_\epsilon(x)\dd{x}=1.\]
                \end{enumerate}
            \end{dfn}
            ...
            \begin{ppt}
                ...
                []
                [Dedicatia]
                设 $\{\delta_\epsilon(x)\}$ 是$\delta$-函数逼近族, 则对于任意的有界\hyperref[dfn:Cont]{连续}函数 $f:\R\to\R$ 有
                \[\lim_{\epsilon\to\epsilon_0}\int_{\R}f(x)\delta_\epsilon(x)\dd{x}=f(0).\]
            \end{ppt}
            \begin{xmp}
                [Gauss-Kernel]
                {Gauss核/热核}
                ...
                [Dedicatia]
                定义Gauss核为函数族
                \[K_\epsilon(x):=\frac{1}{\sqrt{2\pi\epsilon}}\exp(-\frac{x^2}{2\epsilon}).\]
                当 $\epsilon\to 0$ 时这是一个$\delta$-函数逼近族.
            \end{xmp}
            \begin{xmp}
                [poisson-Kernel]
                {poisson核}
                ...
                [Dedicatia]
                定义Poisson核为函数族
                \[K_\epsilon(x):=\frac{1}{\pi}\cdot\frac{\epsilon}{x^2+\epsilon^2}\]
                当 $\epsilon\to 0$ 时这是一个$\delta$-函数逼近族.
            \end{xmp}
            \begin{xmp}
                []
            ...
            \begin{thm}
                ...
                []
                [Dedicatia]
                设 $\{\delta_\epsilon(x)\}$ 是$\delta$-函数逼近族, $f:\R\to\R$ 是 $(a,b)$ 上的有界一致连续函数, 那么 
                \[\forall x\in \Icc{s}{t}\subseteq (a,b), \lim_{\epsilon\to\epsilon_0}f\ast\delta_\epsilon\rightrightarrows f. \]
            \end{thm}
