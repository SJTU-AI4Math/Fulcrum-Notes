    ...
    \section{二次剩余}
        ...
        \subsection{表素数为二平方和}
            ...
            \begin{lma}
                ...
                设\(p:\Z\)为奇素数, 则不定方程
                \[p = x^2 + 2y^2\]
                有解当且仅当\(\left(\frac{-2}{p}\right)=1\)当且仅当\(p\)模\(8\)余\(1\)或\(3\).
            \end{lma}
            ...
            \begin{thm}
                ...
                []
                [白菜]
                设\(p,x,y:\Z\), \(p\)为奇素数, 若\(p\equiv 1 \mod{4}\), 则不定方程\(p=x^2+y^2\)有且仅有一组正整数解满足
                \(0<x<y\)
            \end{thm}
            ...
            \begin{lma}
                ...
                []
                [白菜]
                设\(a,b, x,y,u,v:\Z\), 若\(a=x^2+y^2\), \(b=u^2+v^2\), 则
                \[ab = (xu-yv)^2 + (xv+yu)^2\]
            \end{lma}
            ...
            \begin{thm}
                ...
                []
                [白菜]
                设\(n:\Z\), 则\(n\)能表示为两个整数的平方和的充分必要条件是
                对于任意形如\(4k+3\)的素数\(p\), 若\(p\mid n\), 则\(p\)在\(n\)的素因子分解中出现偶数次.
            \end{thm}
        \subsection{四平方和定理}
            \begin{lma}
                []
                ...
                []
                [白菜]
                设\(a,b, x,y,u,v,w,z:\Z\), 若\(a=x^2+y^2+z^2+w^2\), \(b=u^2+v^2+w^2+z^2\), 则
                \[ab = (xu-yv-zw-zw)^2 + (xv+yu+zw-zw)^2 + (xw-yz+zu-vw)^2 + (xz+yw-vu+wv)^2\]
            \end{lma}
...
\section{数论函数}
    ...
    \section{连分数}
        ...
        \begin{dfn}
            ...
            []
            [白菜]
            设 对于整数列\(\{a_i\}_{i=1}^{\infty}\), 存在正整数 \(k,m\), 使得对任意的 \(n \geq m\), 有
            \[a_n = a_{n+k}\]
            则称由\(\{a_i\}_{i=1}^{\infty}\)生成的连分数为\textbf{循环连分数}.
            该数列生成的无限连分数记作
            \[[a_1,a_2,\cdots,a_{m-1},\dot{a}_m,a_{m+1},\cdots,\dot{a}_{m+k-1}]\]    
        \end{dfn}
        ...
        \begin{thm}
            ...
            []
            [白菜]
            设 对于整数列\(\{a_i\}_{i=1}^{\infty}\), 若其生成的连分数是循环连分数, 则该连分数表示的实数是二次代数数.
        \end{thm}
        ...
        \begin{thm}
            ...
            []
            [白菜]
            设 \(x:\R\). 若 \(x\) 为二次代数数, 则其连分数表示为循环连分数.
        \end{thm}
    \subsection{Pell方程}
        \begin{thm}
            []
            ...
            []
            [白菜]
            设 \(d \in \Z^+\) 为非完全平方数, 则方程
            \[x^2 - d y^2 = 1\]
            有无穷多组整数解 \((x,y)\), 且存在唯一的最小正整数解 \((x_1,y_1)\), 使得任意解均可表示为
            \[
                x_n + y_n \sqrt{d} = (x_1 + y_1 \sqrt{d})^n, n \in \Z^+
            \]
        \end{thm}
    ...
    \section{代数数}
        \subsection{代数数与超越数}
            ...
            \begin{dfn}
                ...
                []
                [白菜]
                设 \(\xi:\C\), 定义 \(\xi\) 为\textbf{超越数}, 当且仅当:
                \[\forall f \in \Q[x], f(\xi) \neq 0\]
            \end{dfn}
        ...
        \subsection{代数数域}
            \begin{thm}
                ...
                []
                [白菜]
                设 \(\A\) 为代数数集合. 则 \(\A\) 关于复数的加法与乘法构成一个\(\C\)的子域.
            \end{thm}

            \begin{prf}
                设\(\alpha, \beta:\A\)设 \(f(x) = x^n + a_{n-1}x^{n-1} + \cdots + a_0\) 是一个有理系数多项式。考虑其友阵为:
                ...
                则\(C(f)\)的特征多项式即为 \(f(x)\). 设\(\alpha, \beta\)的极小多项式分别为 \(f(x), g(x)\).
                则\(\alpha\beta\)即为\(C(f)\otimes C(g)\)的特征值之一, 故代数数的乘法封闭.
                \(\alpha+\beta\)即为\(C(f)\otimes I_1+I_2\otimes C(g)\)的特征值之一, 
                其中\(I_1, I_2\)是单位矩阵, 大小适当选取以使得加法有意义. 故代数数的加法封闭.
                而由于极小性, 若\(\alpha\neq 0\), 则\(C(f)\)可逆, 且逆矩阵也为有理系数矩阵. 
                因此\(\alpha^{-1}\)也是有理系数多项式的根. 于是非零代数数的倒数也是代数数. 
                \(1\)与\(0\)显然分别单位元与零元, 并且也为代数数, 故代数数构成域.
            \end{prf}
        \subsection{代数整数环}
            \begin{dfn}
                [AlgebraicInteger]
                ...
                [Algebraic Integer]
                [白菜]
                设 \(\xi:\C\), 定义 \(\xi\) 为\textbf{代数整数}, 当且仅当:
                \[\exists f \in \Z[x], f \text{首一}\land f(\xi) = 0\]
            ...
            \begin{lma}
                []
                {代数数为代数整数当且仅当极小多项式为整系数多项式}
                []
                [白菜]
                设 \(\xi:\A\), 则 \(\xi\) 为代数整数当且仅当 \(\xi\) 的极小多项式属于 \(\Z[x]\).
            \end{lma}
            ...
            \begin{thm}
                ...
                []
                [白菜]
                设 \(\mathbb{O}\) 为代数整数集合. 则 \(\mathbb{O}\) 关于复数的加法与乘法构成一个环.
            \end{thm}

            \begin{prf}
                依然是考虑极小多项式友阵的张量积. 可以证明加法和乘法的封闭性.
            \end{prf}
        \begin{rmk}
            后面就是代数数论的内容了. 初等数论, 堂堂完结!
        \end{rmk}
