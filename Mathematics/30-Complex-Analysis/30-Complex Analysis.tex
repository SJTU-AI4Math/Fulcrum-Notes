...
\begin{document}
    ...
    \begin{center}
        {\LARGE\textbf{Complex Analysis}}\\[0.5cm]
        {\large F4M Team}\\[0.2cm]        
    \end{center}
...
\section{复数域}
    ...
    \subsection{复平面的紧致化}
        ...
        \begin{dfn}
            ...
            [Dedicatia]
            称通过球极投影建立复数与Riemann球面之间的对应关系所确定的复数全体为\textbf{扩充复平面}. \\
            记作 \(\barC\).\\
            \(\barC=\C\cup\{\infty\}\).
        \end{dfn}
...
\section{复函数}
    ...
    \subsection{全纯函数}
        ...
        \begin{dfn}
            ...
            容易发现, \全纯 的实部 \(u(x,y)\) 和虚部 \(v(x,y)\) \textbf{都是调和函数}. 
        \end{dfn}
        ...
        \begin{thm}
            ...
            []
            [Dedicatia]
            设 \(u(x,y)\) 和 \(v(x,y)\) 具有适合于Cauchy-Riemann方程的一阶连续偏导数, 那么 \(f(z)=u(x+\ii y)+\ii v(x+\ii y)\) 是\全纯. 
        \end{thm}
    ...
    \subsection{解析函数}
        ...
        \begin{rmk}
            [Dedicatia]
            只有在复数域上, 全纯函数才等价于解析函数. 在实数域上, 这二者并不等价. 例如, 函数 \(f(x)=e^{-\frac{1}{x^2}}\) 在 \(x=0\) 处的Taylor级数恒为0, 但 \(f(x)\) 并不恒为0.\\
            由此可见, 复数域有着更强的约束力.
        \end{rmk}
...
\section{作为映照的解析函数}
    ...
    \subsection{共形映照}
        ...
        \begin{thm}
            ...
            \[S(f(D))=\iintop_{D}|f'(z)|^2\dd{x}\dd{y}.\]
        \end{thm}
    ...
    \subsection{复平面射影几何}
        ...
        \begin{dfn}
            ...
            [Dedicatia, 猫猫]
            定义 \(\barC\to\barC\) 上\textbf{线性分式变换}为 \(\barC\to\barC\) 的子类型: 
            \[\left\{z\mapsto\frac{az+b}{cz+d}:\barC\to\barC\middle|a,b,c,d:\C, ad-bc\neq 0\right\}\]
        ...
        \begin{rmk}
            [猫猫]
            事实上, M\"obius 变换已然占据了全部 \(\barC\) 上的全纯双射, 即: 
            \[\Mobius\cong\Aut(\barC)\]
        \end{rmk}
        ...
        \begin{dfn}
            ...
            [Cross Ratio]
            [Dedicatia, 猫猫]
            设 \(z_1,z_2,z_3,z_4:\barC\), 定义 \(z_1,z_2,z_3,z_4\) 的\textbf{交比}为: 
        ...
        \begin{ppt}
            ...
            []
            [Dedicatia, 猫猫]
            设 \(f:\Mobius\), \(z_1,z_2,z_3,z_4:\barC\), 则: 
    ...
    \subsection{初等解析函数}
        ...
        \begin{dfn}
            ...
            [Dedicatia]
            若 \(w=x+\ii y\), 那么总可以将其写成三角形式, 根据Euler公式可以将其写成 \(\exp z\) 的形式. 这时就可以用对数来表示 \(z\). 也就是
            \[\Ln w=\ln|w|+\ii\Arg w (w\neq 0)\]
            值得注意的是, \textbf{这是一个\hyperrefc{dfn:Multivalued}{多值函数}! }为了和一般的实对数区别, 我们记作 \(\Ln w\). 这时的每一组值称为一个\textbf{解析分支}. 如果我们取其中的一个分支, 规定 \(\arg w\in[0,2\pi)\), 就可以保证其是单值的, 这时可以使用 \(\ln w\):
            \[\ln w=\ln|w|+\ii\arg w (w\neq 0)\]
        \end{dfn}
        ...
        \begin{dfn}
            ...
            这时, 除非 \(\alpha\) 是正的实整数, 否则幂函数都是\hyperrefc{dfn:Multivalued}{多值函数}. 
        \end{dfn}
...
\section{复积分理论}
    ...
    \subsection{Taylor定理与解析函数的奇点}
        ...
        \begin{dfn}
            ...
            [Meromorphic Function]
            [Dedicatia, 猫猫]
            设 \(D\) 是区域, \(f:D\to\barC\), 定义 \(f\) 是\textbf{亚纯函数 / 半纯函数}, 当且仅当: \(\forall z\in D, z\) 不是极点 \(\implies f\) 在 \(z\) 处解析. 
        \end{dfn}
        ...
        \begin{crl}
            ...
            []
            [Dedicatia]
            有理分式 \(\frac{f(z)}{g(z)}\), 其中 \(g(z)\) 不恒为0, 那么它都是\亚纯.
        \end{crl}
        ...
        \begin{thm}
            {本性奇点的Weierstass条件}
            \(z_0\) 是 \(f(z)\) 的本性奇点的充要条件是:  \(\forall\,A\in\barC\), 都能在 \(z_0\) 的邻域内找到一列收敛到 \(z_0\) 的序列 \(\{z_n\}\), 使得 
    ...
    \subsection{Schwarz引理与双曲度量}
        ...
        \begin{dfn}
            ...
            [Unit Disk]
            [Dedicatia]
            复平面上的开集 \(\{z:|z|<1\}\) 称为\textbf{单位圆盘}. 用记号 \(\D\) 表示.
        \end{dfn}
        ...
        \begin{thm}
            ...
            [Schwarz Lemma]
            [Dedicatia]
            设函数 \(f(z)\) 在 \(\D\) 上解析, 且满足 \(|f(z)|\leq 1\). 如果 \(f(0)=0\), 那么对任意的 \(z\) 都有
            \[|f(z)|\leq |z|\]
            并且
            \[|f'(0)|\leq 1\]
            如果在某一点 \(z_0\neq 0\) 处取等号, 或者在 \(f'(0)\) 处取等号, 那么存在一个实数 \(\theta\), 使得
            \[f(z)=\exp(\ii\theta)z\]
        \end{thm}
        ...
        \begin{dfn}
            ...
            这里的上确界是对所有将 \(D\) 映射到单位圆盘 \(|w|<1\) 上的解析函数 \(f(z)\) 取的. 那么称 \(\rho_D(z)|\dd{z}|\) 为区域 \(D\) 上的\textbf{双曲度量}.
        \end{dfn}
        ...
        \begin{thm}
            ...
            []
            [Dedicatia]
            设 \(f:\D\to\D\) 是一个解析映射, 那么对任意的 \(z\in\D\), 都有
            \[\frac{|f'(z)|}{1-|f(z)|^2}\leq \frac{1}{1-|z|^2}\]
            如果在某一点 \(z_0\) 处取等号, 那么存在一个实数 \(\theta\), 使得
            \[f(z)=\exp(\ii\theta)\frac{z+z_0}{1+\overline{z_0}z}.\]
            即 \(f\) 是 \(\D\) 上的一个解析自同构.\\
            Schwarz-Pick 定理也可以写作: 设 \(f:\D\to\D\) 是一个解析映射, 那么 \(\forall z_1,z_2\in\D\), 
            \[\vqty{\frac{f(z_1)-f(z_2)}{1-\overline{f(z_2)}f(z_1)}} \leq \vqty{\frac{z_1-z_2}{1-\overline{z_2}z_1}}\]
        \end{thm}
    ...
    \subsection{局部映射与幅角原理}
        ...
        \begin{crl}
            ...
            [Argument Principle]
            [Dedicatia]
            设 \(f(z)\) 是 \(D\) 上的\亚纯, 具有 \(m\) 个零点 \(a_j\) 和 \(n\) 个极点 \(b_k\),  \(\gamma\) 是 \(D\) 上的\hyperrefc{dfn:NullHomologous}{同调于零}的不过 \(a_j\) 与 \(b_k\) 的闭链, 有
        ...
        \begin{crl}
            ...
            []
            [Dedicatia]
            设 \(f(z)\) 是 \(D\) 上的\亚纯, 具有 \(m\) 个零点 \(a_j\) 和 \(n\) 个极点 \(b_k\),  \(\gamma\) 是 \(D\) 上的\hyperrefc{dfn:NullHomologous}{同调于零}的不过 \(a_j\) 与 \(b_k\) 的闭链,  \(g(z)\) 是定义在 \(D\) 上的\解析, 有
...
\section{函数的级数展开与因子分解}
    ...
    \subsection{Taylor 级数与 Laurent 级数}
        ...
        \begin{rmk}
            [Dedicatia]
            从本质上看, Laurent级数就是两个幂级数拼接而成的, 这时要分析收敛性, 就需要分别分析两个幂级数的收敛性.
        \end{rmk}
        ...
        \begin{xmp}
            ...
            计算函数
            \[f(z)=\frac{1}{z(z-2)^3}\]
            在 \(z=2\) 处的Laurent级数系数 \(a_{-1},a_{0},a_{1}\).\\
            根据\hyperref[thm:Laurent-Series]{Laurent级数}的系数计算公式, 有
            \[a_n=\frac{1}{2\pi\ii}\oint_{\gamma}\frac{f(\zeta)}{(\zeta-2)^{n+1}}\dd{\zeta}\]
            其中 \(\gamma\) 是包含 \(z=2\) 的小圆周. 那么
            \[a_{-1}=\frac{1}{2\pi\ii}\oint_{\gamma}\frac{f(\zeta)}{(\zeta-2)^{0}}\dd{\zeta}=\frac{1}{2\pi\ii}\oint_{\gamma}\frac{1}{\zeta(\zeta-2)^3}\dd{\zeta}\]
            根据留数定理, 该积分等于 \(2\pi\ii\) 乘以 \(z=2\) 处的留数. 计算留数:
            \[\Res(f,2)=\lim_{z\to 2}\dv[2]{z}((z-2)^3f(z))=\lim_{z\to 2}\dv[2]{z}(\frac{1}{z})=\frac{2}{8}=\frac{1}{4}\]
            所以 \(a_{-1}=\frac{1}{4}\).
        \end{xmp}
        ...
        \begin{xmp}
            ...
            []
            [Dedicatia]
            由于当$|x|<1$时，$\sum\limits_{n=0}^{\infty}x^n=\frac{1}{1-x}$，所以对于某个收敛半径也是1的幂级数来说，当$|x|<1$时，有
            \[\frac{1}{1-x}\sum_{n=0}^{\infty}a_nx^n=\sum_{n=0}^{\infty}S_nx^n,\]
            $S_n=\sum\limits_{l=0}^na_l$.

            所以我们得到了如下的式子：
            \begin{align*}
                \frac{1}{(1-x)^2}&=\sum_{n=0}^{\infty}(n+1)x^n;\\
                \frac{1}{(1-x)^3}&=\sum_{n=0}^{\infty}\frac{(n+1)(n+2)}{2}x^n;\\
                \frac{1}{(1-x)^4}&=\sum_{n=0}^{\infty}\frac{(n+1)(n+2)(n+3)}{6}x^n;\\
                &\cdots 
            \end{align*}
            可以用来计算Laurent级数的系数.
        \end{xmp}
    ...
    \subsection{亚纯函数的部分分式展开}
        ...
        我们从\hyperrefc{thm:Laurent-Series}{Laurent级数}中可以看到: 如果 \(f(z)\) 是一个\亚纯, 对极点 \(b_n\), 就会有 \(f(z)\) 的非解析部分 \(P_n\qty(\frac{1}{z-b_n})\). 如此减去所有的非解析部分之后就会得到解析的函数 \(g(z)\). 也就是说我们可以将 \(f(z)\) 分解为
        \[f(z)=\sum_{i=1}^{n}P_n\qty(\frac{1}{z-b_n})+g(z)\]
        但是这样定义,  \(P_n\) 往往是无限项, 还有可能不收敛. 所以我们改进: 

        \begin{thm}
            {亚纯函数的部分分解定理}
            设 \(\{b_n\}\) 是一个复数序列,  \(\lim_{n\to\infty }b_n=\infty\), 设 \(P_n\) 是不含常数项的多项式, 那么存在一些在全平面的\亚纯 \(f(z)\), 其以 \(b_n\) 为\hyperref[dfn:Pole]{极点}, 非解析部分为所有的 \(P_n\qty(\frac{1}{z-b_n})\), 使得
...
\section{Riemann映照定理}
    ...
    \subsection{Riemann映照定理与边界表现}
        ...
        \begin{thm}
            ...
            [Dedicatia]
            给定 \(D\subsetneqq\C\) 是单连通区域和 \(z_0\in D\), 在 \(D\) 上存在唯一一个解析函数 \(f\) 满足 \(f(z_0)=0, \arg f'(z_0)=\theta_0\), 使得 \(f(z)\) 将 \(D\) 映到圆盘 \(\D\), 且 \(f\) 是\hyperref[dfn:Conformal-Mapping]{共形映照}.
        \end{thm}

        \begin{prf}
            固定 \(z_0\in D\). 对任意解析单射 \(f:D\to\D\): 构造函数族
            \[\mathcal{F}:=\{f:D\to\D,f(z_0)=0 \}.\]
            定义 
            ...
            设 \(\mathcal{F}\) 中的函数列 \(\{f_n\}\) 在 \(D\) 上紧致收敛到 \(f_0\). 由于 \(f\) 都是解析单射, 如果 \(f_0\) 是常解析函数就会使 \(\lim\limits_{n\to+\infty}f'_n=0\). 这是不可能的. 所以 \(f_0:U\to\D\) 也是解析单射.\\
            ...
            接下来考虑 \(f_0\) 的像. 反设: \(f(D)=\Omega\subsetneqq\D\). 那么取 \(w_1\in\D\setminus\Omega\) 构造 M\"obius 同构
            ...
            这说明 \(|\tilde{g}'(z_0)|\) 超过了上确界 \(\alpha\). 显然不可能发生. 这说明不存在这样的 \(w_1\). 即 \(f(D)=\D\), \(f_0\) 是满射, 因而 \(f_0\) 是解析双射, 也就是共形映射.\\
            最后证明其唯一性: 如果 \(f,g\) 都是 \(D\to\D\) 上的共形映射且 \(f(z_0)=g(z_0)=0\), \(\arg f'(z_0)=\arg g'(z_0)=\theta_0\), 那么 \(f=g\). 证明 \(g^{-1}\circ f=\mathbf{1}\). 使用 Schwarz 引理可知 \(g^{-1}\circ f:\D\to\D=\exp(\ii t)\). 由于 \(\arg f'(z_0)=\arg g'(z_0)\) 的限制, 得到 \(\exp(\ii t)=1\) 恒成立. 所以 \(f=g\) 恒成立. 这就证明了共形映射是唯一的.
        \end{prf}
        ...
        \begin{thm}
            ...
            []
            [Dedicatia]
            设区域 \(D\) 的边界 \(\gamma\) 是Jordan曲线, \(f:D\to\D\) 是\ConformalMapping. 那么 \(\exists\,\tilde{f}:\overline{D}\to\overline{\D}\), \(\tilde{f}\) 是一个\ConformalMapping.
        \end{thm}
        ...
        \begin{xmp}
            ...
            \begin{enumerate}
                \item 将上半平面 \(\mathbb{H}\) 映到单位圆盘 \(\D\) 的\textbf{M\"obius变换}: \[w=\frac{z-z_0}{z-\overline{z_0}}, \Im z_0>0\]
                \item 将单位圆盘 \(\D\) 映到上半平面 \(\mathbb{H}\) 的\textbf{M\"obius变换}: \[w=\ii\frac{1+z}{1-z}\]
                \item 将扇形 \(0\leq\arg z\leq \alpha\) 映到上半平面 \(\mathbb{H}\) 的\textbf{幂函数}: \[w=z^{\pi/\alpha}\]
                \item 将水平条 \(\{z:a<\Im z<b\}\) 映到上半平面 \(\mathbb{H}\) 的\textbf{指数函数}: \[w=\exp\frac{\pi}{b-a}z\]
                \item 将上半平面 \(\mathbb{H}\) 映到水平条 \(\{z:a<\Im z<b\}\) 的\textbf{对数函数}: \[w=\frac{b-a}{\pi}\ln z\]
                \item 将半无穷条带 \(0\leq\Re z\leq\pi,\Im z>0\) 映到上半平面 \(\mathbb{H}\) 的\textbf{三角函数}: \[w=\sin z, w=\cos z, w=\sinh z, w=\cosh z\] 具体使用哪个要视情况而定.
                \item \(\D\) 的自同构变换: \[w=\exp(\ii\theta)\frac{z-a}{1-\overline{a}z}, |a|<1, \theta\in\R\] 这会将复数 \(a\) 移动到原点.
                \item 上半平面 \(\mathbb{H}\) 的自同构变换: \[w=\frac{az+b}{cz+d}, a,b,c,d\in\R, ad-bc>0\]
            \end{enumerate}
        \end{xmp}
    ...
    \subsection{多边形的共形映射}
        ...
        \begin{thm}
            []
            {将上半平面映到矩形的Schwarz-Christoffel变换}
            []
            [Dedicatia]
            存在唯一的共形映射 \(f\) 将上半平面映到矩形区域, 使得 \(f\) 将实轴上的点 \(0,1,\frac{1}{k^2},\infty\) 分别映到矩形的四个顶点, 其中 \(0<k<1\). 那么 \(f\) 的表达式为
            \[f(z)=C\int_{0}^{z}\frac{\dd{\xi}}{\sqrt{\xi(1-\xi)(1-k^2\xi)}}+C_0\]
            其中 \(C,C_0\in\C\) 是常数.\\
            具体而言:\begin{itemize}
                \item 区间 \([0,1]\) 映到矩形的下边 \([0,K(k)]\);
                \item 区间 \([1,\frac{1}{k^2}]\) 映到矩形的右边 \([K(k),K(k)+\ii K'(k)]\);
                \item 区间 \([\frac{1}{k^2},+\infty)\) 映到矩形的上边 \([K(k)+\ii K'(k),\ii K'(k)]\);
                \item 区间 \((-\infty,0]\) 映到矩形的左边 \([\ii K'(k),0]\).
            \end{itemize}
        \end{thm}
        ...
        \begin{dfn}
            ...
            [Conformal Modulus]
            [Dedicatia]
            设单连通区域 \(D\) 通过Schwarz-Christoffel变换映到了矩形区域 \([0,1]\times[0,m]\), 那么称 \(m\) 为区域 \(D\) 的\textbf{共形模量}.\\
            其中:
            \[m=\frac{K'(k)}{K(k)}.\]
        \end{dfn}
    ...
    \subsection{反射原理}
        ...
        \begin{thm}
            ...
            []
            [Dedicatia]
            设区域 \(D\) 被一条实轴上的直线 \(L\) 分成两部分 \(D^+\) 和 \(D^-\), 且 \(D^-\) 是 \(D^+\) 关于 \(L\) 的镜像. 如果函数 \(f\) 在 \(D^+\) 上是\亚纯 且在 \(L\) 上取实值, 那么可以定义函数 \(\tilde{f}\) 在整个区域 \(D\) 也是\亚纯, 其定义为
            \[\tilde{f}(z)=\begin{cases}
                f(z), & z\in D^+\\
                \overline{f(\bar{z})}, & z\in D^-
            \end{cases}\]
...
\section{椭圆函数}
    ...
    \subsection{椭圆函数的引入}
        ...
        \begin{dfn}
            ...
            [Doubly Periodic Function]
            [Dedicatia]
            设复函数 \(f:\C\to\C\), 如果存在两个实线性无关的复数 \(\omega_1,\omega_2\), 即不存在 \(a\in\R\) 满足 \(\omega_1=a\omega_2\), 使得对于任意的 \(z\in\C\), 都有
            \[f(z+\omega_1)=f(z),\quad f(z+\omega_2)=f(z),\]
            那么就称 \(f\) 是一个\textbf{双周期函数}. 并定义 \(\omega_1,\omega_2\) 为 \(f\) 的\textbf{基本周期}.
        \end{dfn}
        ...
        \begin{dfn}
            ...
            []
            [Dedicatia]
            设双周期函数的基本周期为 \(\omega_1,\omega_2\). 那么由 \(\omega_1,\omega_2\) 张成的平行四边形区域
            \[D:=\{z=t\omega_1+s\omega_2:t,s\in[0,1]\}\]
            称为该双周期函数的\textbf{基本周期区域}.
        \end{dfn}
        ...
        \begin{ppt}
            ...
            []
            [Dedicatia]
            设 \(f:\C\to\C\) 是双周期函数且同时是解析函数. 那么 \(f\) 一定是常解析函数.
        \end{ppt}

        \begin{prf}
            设 \(D\) 是由周期 \(\omega_1,\omega_2\) 张成的平行四边形区域. 由于 \(f\) 是双周期函数, 所以 \(f\) 在整个复平面上都由其在 \(D\) 上的值唯一确定. 由于 \(f\) 在 \(D\) 上解析且 \(D\) 是紧致集, 所以 \(f\) 在 \(D\) 上有界. 根据最大模原理可知, \(f\) 在整个复平面上有界. 根据 Liouville 定理可知, \(f\) 一定是常解析函数.
        \end{prf}
        ...
        \begin{ppt}
            ...
            []
            [Dedicatia]
            设 \(f:\C\to\C\) 是双周期函数且在基本周期区域 \(D\) 内只有有限个孤立奇点 \(a_1,a_2,\cdots,a_n\). 那么有
            \[\sum_{k=1}^{n}\Res(f,a_k)=0.\]
        \end{ppt}
        ...
        \begin{dfn}
            ...
            [Elliptic Function]
            [Dedicatia]
            定义复函数 \(f\) 是椭圆函数, 当且仅当 \(f\) 是双周期函数且在基本周期区域内是\亚纯.
        \end{dfn}
        ...
        \begin{dfn}
            ...
            [Order]
            [Dedicatia]
            定义椭圆函数的\textbf{阶}为其在基本周期区域内的极点个数, 计重数.
        \end{dfn}
        ...
        \begin{ppt}
            ...
            []
            [Dedicatia]
            设 \(f\) 是椭圆函数, 那么 \(f\) 的阶数至少为 2.\\
            不存在阶数为 1 的椭圆函数.
        \end{ppt}

        \begin{prf}
            反设 \(f\) 是阶数为 1 的椭圆函数, 那么在基本周期区域内只有一个极点 \(a\). 根据前面的结论可知, 该极点的留数为零. 这说明该极点是不可能是单极点, 因为 \(\frac{1}{z}\) 的奇性使得留数不可能为0. 这就出现了矛盾. 所以椭圆函数的阶数至少为 2.
        \end{prf}

        \begin{rmk}
            \textbf{椭圆函数的得名}是起源于对椭圆积分的反函数的研究. 椭圆积分是指形如
            \[\int R\left(x,\sqrt{P(x)}\right)\dd{x}\]
            的积分, 其中 \(R\) 是有理函数, \(P(x)\) 是三次或四次多项式且无重根. 这些积分最初出现在计算椭圆的弧长时. \\
            圆函数 \(\sin x\) 和 \(\cos x\) 可以看作是计算单位圆弧长积分
            \[\int_{0}^{x}\frac{\dd{t}}{\sqrt{1-t^2}}\]
            的反函数. 类似地, 通过研究
            \[\int_{0}^{x}\frac{\dd{t}}{\sqrt{(1-t^2)(1-k^2t^2)}},\quad k\in(0,1)\]
    ...
    \subsection{Weierstrass \(\wp\) 函数}
        ...
        \begin{dfn}
            ...
            [Dedicatia]
            设复数 \(\omega_1,\omega_2\) 实线性无关, 定义点族
        ...
        \begin{rmk}
            [Dedicatia]
            Weierstrass 直接从构造的角度出发. 为了找到一个椭圆函数的显式表达式, 就考虑构造一个在复平面上有周期排列的极点的亚纯函数: 由于椭圆函数的极点必须至少是二阶, 所以考虑构造一个在每个周期点都有二阶极点的函数. \\
            最直观的想法就是将函数 \(\frac{1}{z^2}\) 沿着周期点进行平移, 并将这些平移后的函数进行叠加:
            \[\sum_{\omega\in\Lambda}\frac{1}{(z-\omega)^2}.\]
            但是该级数并不在 \(\C\) 上收敛, 因为其项的绝对值与 \(\sum_{\omega\in\Lambda}\frac{1}{|\omega|^2}\) 同阶, 而后者是发散的. 为了使得该级数收敛, 可以对每一项进行适当的调整, 即减去 \(\frac{1}{\omega^2}\), 使得其在无穷远处的行为更好一些. 经过调整后得到 Weierstrass \(\wp\)-函数的定义:
            \[\wp(z)=\frac{1}{z^2}+\sum_{\omega\in\Lambda\setminus\{0\}}\left(\frac{1}{(z-\omega)^2}-\frac{1}{\omega^2}\right).\]
        \end{rmk}
