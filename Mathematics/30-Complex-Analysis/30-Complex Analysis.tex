...
\section{复函数}
    ...
    \subsection{复函数项级数}
        ...
        \begin{crl}
            ...
            [Dedicatia]
            设 \(D\) 上的函数项级数 \(f_n\), 则 \(f_n\) 在 \(D\) 上\一致收敛 当且仅当: 
    ...
    \subsection{幂级数}
        ...
        \begin{thm}
            ...
            \begin{enumerate}
                ...
                \item 当 \(|z|<R\) 时的和是\解析, 根据\一致收敛 性, 其导数可以逐项微分求得. 
            \end{enumerate}
            这个 \(R\) 由公式
...
\section{复积分理论}
    ...
    \subsection{高阶导数与解析函数的性质}
        ...
        \begin{crl}
            ...
            []
            [Dedicatia]
            如果解析函数列 $\{f_n\}$ 在区域 $D$ 上内闭一致收敛, 那么其任意阶导数 $\{f_n^{(k)}\}$ 在 $D$ 上也内闭一致收敛.
        \end{crl}
...
\section{函数的级数展开与因子分解}
    ...
    \subsection{Weierstrass定理}
        ...
        \begin{thm}
            ...
            [Hurwitz's Theorem]
            [Dedicatia]
            设函数 \(f_n(z)\) 在 \(D\) 上解析且不是常数, 并设 \(f_n(x)\) 在 \(D\) 的任意紧致子集上\一致收敛 于 \(f(x)\), 那么 \(f(z)\) 在 \(D\) 上或者恒等于0, 或者恒不等于0.
        \end{thm}
