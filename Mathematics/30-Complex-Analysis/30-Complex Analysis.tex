    ...
    \subsection{幂级数}
        ...
        \begin{thm}
            ...
            \begin{enumerate}
                ...
                \item 当 \(|z|<R\) 时的和是\解析, 根据\UniformConvergence 性, 其导数可以逐项微分求得. 
            \end{enumerate}
            这个 \(R\) 由公式
...
\section{作为映照的解析函数}
    ...
    \subsection{共形映照}
        ...
        \begin{ppt}
            ...
            []
            [Dedicatia]
            如果一个\解析 在某定义域 \(\Omega\) 上导数恒为0, 那么它必定是常函数. 
        \end{ppt}
...
\section{函数的级数展开与因子分解}
    ...
    \subsection{Weierstrass定理}
        ...
        \begin{thm}
            ...
            设级数 \(\sum_{n = 1}^{\infty} u_n(z)\) 的每一项都是\解析, 且在 \(D\) 的任意紧致子集 \(D'\) 上\UniformConvergence, 那么级数的和函数 \(S(z)\) 在 \(D\) 上是解析函数, 且可以逐项微分. 
    ...
    \subsection{Taylor级数与Laurent级数}
        ...
        \begin{thm}
            ...
            [Laurent's Series]
            [Dedicatia]
            设在圆环 \(R_1<|z-z_0|<R_2(R_1\geqslant 0, R_2\leqslant+\infty )\) 内的\解析  \(f(z)\) 必定可以写成同时包含正幂和负幂的级数: 
